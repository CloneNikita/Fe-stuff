...
local messages = {
	"You are playing: 15 Sec Guess, edited chat by Lordy",
}
local chatted = {}
local points = {}
local rounds = 5
...
local function updateSign(text: string, category: string, round: number, timeLeft: number, icon: number)
	boothUpdate:FireServer("Update", {
		["DescriptionText"] = "[🕹️] 15 sec guess ("..tostring(round).."/"..tostring(rounds)..")\n[🔤] "..text.."\ncategory: "..category.."\n[⏰]"..timeLeft,
		["ImageId"] = icon,
	})
end
...
while true do
	...
	local words = game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/CloneNikita/Fe-stuff/main/words.json", true))
	...
	boothUpdate:FireServer("Update", {
		["DescriptionText"] = leaderboardTable[1].Name.." wins with 💠 "..points,
		["ImageId"] = 9731042021,
	})
	task.wait(5)
end
